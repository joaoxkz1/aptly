 ...
 * Usage: setup | serve | run CASE_ID | status | cleanup-check | cleanup
 */
...
const MAX_STARTED_ATTEMPTS = 10;
const backend = "http://127.0.0.1:54321", appOrigin = "http://127.0.0.1:3122", proxyOrigin = "http://127.0.0.1:3123";
const root = resolve("docs/examiner-live-validation"), records = join(root, "attempts");
const statePath = resolve(".env.examiner-live-state.json"), activePath = resolve(".env.examiner-live-active.json"), lockPath = resolve(".env.examiner-live-run.lock");
const budgetPath = join(root, "budget.json");
const mode = process.argv[2];
if (!["setup", "serve", "run", "status", "cleanup-check", "cleanup"].includes(mode)) throw new Error("Use setup, serve, run CASE_ID, status, cleanup-check or cleanup");
...
if (mode === "setup") {
  ...
  const proxy=createServer(async(req,res)=>{
    try {
      ...
      const stage=name==="aptly_assessed_diagram_observations"?"observer":name==="aptly_grade_result"?"grader":name==="aptly_manual_task_interpretation"?"task-resolution":null;
  ...
  const entry=model.bankQuestionId ? ECONOMICS_QUESTION_BANK.find(q=>q.id===model.bankQuestionId) : null;
  if(typeof model.answer!=="string" || typeof model.question!=="string" || (model.bankQuestionId && (!entry || model.question!==entry.question)) || !Array.isArray(test.artifacts) || test.artifacts.length>1)throw new Error("Invalid model input");
  if(!entry && !["paper1a_10_mark","paper1b_15_mark","paper2g_15_mark"].includes(model.framework))throw new Error("Invalid manual framework");
  const task=entry ?? {question:model.question,framework:model.framework,marks:model.framework==="paper1a_10_mark"?10:15,sourceMaterial:model.sourceMaterial??null,topicCode:"unknown",levelRelevance:"shared_sl_hl"};
  ...
  try{
    ...
    if(b.attempts.length>=MAX_STARTED_ATTEMPTS)throw new Error("Absolute 10-attempt limit reached");
    ...
    const authored=entry ? entry.gradingBlueprint.kind==="extended"?{...entry.gradingBlueprint,diagramRequirement:ESSAY_DIAGRAM_AUDIT[entry.id]}:entry.gradingBlueprint : undefined;
    const blueprintVersion=entry ? entry.gradingBlueprint.kind==="extended"?"economics-essay-blueprint-v2":entry.gradingBlueprintVersion : undefined;
    const question=entry ? await savePracticeQuestion(account.userId,key,{question:entry.question,sourceMaterial:entry.sourceMaterial??null,framework:entry.framework,markTotal:entry.marks,
      ...
      fromCurrentFocus:false,requestFingerprint:hash(key)}) : null;
    const contract=resolveAssessmentContract({policy:policyForGeneratedPractice({framework:task.framework,markTotal:task.marks,sourceMaterial:task.sourceMaterial??null}),question:task.question,sourceMaterial:task.sourceMaterial??null,
      topic:task.topicCode,blueprint:authored,blueprintVersion,level:task.levelRelevance,bankQuestionId:entry?.id});
    const implementationHashes=Object.fromEntries(["lib/ai/combined-assessment.ts","lib/ai/examiner-judgment.ts","lib/ai/manual-task-resolution.ts","lib/assessment/examiner-task-registry.ts","lib/ai/assessed-visual-review.ts","lib/assessment/component-scoring.ts","app/api/grade/route.ts"].map(path=>[path,hash(readFileSync(path))]));
    write(join(folderPath,"resolved-task.json"),{question:task.question,sourceMaterial:task.sourceMaterial??null,answer:model.answer,initialContract:contract,bankQuestionId:entry?.id??null,imagePath:selectedImage??null,imageHash:image?hash(image):null,implementationHashes});
    const auth=await cookieClient(account),payload={subject:"Economics",topic:task.topicCode,question:task.question,answer:model.answer,practiceQuestionId:question?.id??null,parentAttemptId:null,idempotencyKey:operationKey,diagramOmitted:!image,...(!entry?{requestedFramework:task.framework,sourceMaterial:task.sourceMaterial??null}:{})};
    ...
    slot.status=result.ok&&row.data?"complete":"incomplete";slot.httpStatus=result.status;slot.finalMark=response.attempt?.assessment?.marksEarned??null;slot.total=task.marks;slot.latencyMs=duration;
    console.log(JSON.stringify({number,caseId,status:slot.status,httpStatus:result.status,mark:slot.finalMark,total:task.marks,latencyMs:duration,record:folder}));
...
} else if(mode === "cleanup" || mode === "cleanup-check") {
  if(existsSync(lockPath))throw new Error("An assessment is still running; do not clean up");
  const s=state();
  // Prove every target was created by this exact local validation setup before
  // deleting either one. Never follow ambient credentials to production.
  if(s.accounts.length!==2 || !budget().closedAt)throw new Error("Closed local run with exactly two fixture accounts required");
  for(const a of s.accounts){
    const found=await admin.auth.admin.getUserById(a.userId),u=found.data.user;
    if(found.error || !u || u.email!==a.email || !/^aptly-live-validation-[0-9a-f-]+@example\.test$/.test(u.email)
      || u.user_metadata?.display_name!==`Live validation ${a.label.toUpperCase()}`
      || new Date(u.created_at)<new Date(s.createdAt) || new Date(u.created_at)-new Date(s.createdAt)>60000) throw new Error("Cleanup target provenance not established");
  }
  console.log(JSON.stringify({verifiedEndpoint:backend,exactSetupCreatedAt:s.createdAt,verifiedSetupAccounts:s.accounts.length,matchingFixtureEmailAndMetadata:true,allCreatedWithinSetupMinute:true,productionAccountsAffected:0,mode}));
  if(mode==="cleanup") {
    for(const a of s.accounts){const deleted=await admin.auth.admin.deleteUser(a.userId);if(deleted.error)throw new Error("Local cleanup failed");}
