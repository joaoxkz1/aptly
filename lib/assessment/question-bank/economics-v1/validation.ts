...
const EXPLICIT_VISUAL = /\b(draw|sketch|plot|using|with the aid of|refer to)\b[^.?!]{0,45}\b(diagram|graph|chart|figure)\b/i;
...
export function validateEconomicsQuestionBank(
  ...
  for (const question of bank) {
    ...
    if (![2, 10, 15].includes(question.marks)) errors.push(`${prefix}: unsupported marks`);
    ...
    if (EXPLICIT_VISUAL.test(question.question)) errors.push(`${prefix}: explicit visual requirement`);
    if (SOURCE_DEPENDENCY.test(question.question)) errors.push(`${prefix}: source dependency`);
